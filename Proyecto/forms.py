...
class LoginForm(FlaskForm):
	username = StringField('Usuario', validators=[DataRequired(message='Llena el campo.')])
	password = PasswordField('Clave', validators=[DataRequired(message='Llena el campo.')])
...
class RegistrationForm(FlaskForm):
	...
	def validate_email(self, email):
		email = Usuarios.query.filter_by(Correo=email.data).first()
		if email is not None:
			raise ValidationError('Por favor utilice otra dirección de correo electrónico.'.decode('utf-8'))

class CuentaForm(FlaskForm):
	nombre = StringField('Nombre', validators=[DataRequired(message='Llena el campo.')])
	saldo = DecimalField('Saldo', validators=[DataRequired(message='Llena el campo.')])
...
class CreditoForm(FlaskForm):
	fecha_pago = StringField('Fecha_Pago', validators=[DataRequired(message='Llena el campo.')])
	limite = DecimalField('Limite', validators=[DataRequired(message='Llena el campo.')])
	submit = SubmitField('Crear')
...
class PrestamoForm(FlaskForm):
	valor = DecimalField('Valor_Prestamo', validators=[DataRequired(message='Llena el campo.')])
	cuotas = IntegerField('Cuotas', validators=[DataRequired(message='Llena el campo.')])
	TAE = DecimalField('TAE', validators=[DataRequired(message='Llena el campo.')])
	fecha_pago = StringField('Fecha_Pago', validators=[DataRequired(message='Llena el campo.')])
	submit = SubmitField('Crear')
